...
def _looks_like_target(value, *, allow_selection=False):
    ...
    if not allow_selection and text.startswith(("-", "※", "■", "①", "②", "③")):
        # 모집단위 칸에 설명/주석 문장이 들어온 경우를 막는다.
        return False
...
def normalize_safe_csat_minimum_rule(rule):
    """공식 Q1 파싱 결과 중 수능최저로 확실한 규칙만 통과시킨다.

    ADIGA의 복잡한 표에서는 '지원자격', '1단계', 모집단위명 자체가
    수능최저 열처럼 확장되는 경우가 있다. 사이트 신뢰성을 위해 애매한 행은
    저장하지 않고, 명시적인 없음/적용 문구나 구체적인 등급 기준만 보존한다.
    """
    if rule is None:
        return None
