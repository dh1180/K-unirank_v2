...
def _inject_detail_assets(html):
    css_url = escape(static("css/admissions-metric-refine.css") + "?v=20260915-1", quote=True)
    css_tag = f'<link rel="stylesheet" href="{css_url}">'
    if "</head>" in html:
        html = html.replace("</head>", css_tag + "\n</head>", 1)

    if "</body>" in html:
        script_url = escape(static("js/admissions-detail-mobile.js") + "?v=20260915-1", quote=True)
